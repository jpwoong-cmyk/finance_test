...
function openNewAccountDrawer(section) {
  ...
  setDrawerTheme(section);
  window.NeonStatementImport?.reset();
  showDrawer();
...
function configureStatementTools(section, item) {
  ...
  if (window.NeonStatementImport) {
    window.NeonStatementImport.configure(section, item);
    return;
  }

  // Safe fallback if the separate statement module has not loaded yet.
  if (tools) tools.hidden = section !== 'expenses';
}
...
window.NeonFinanceApp = Object.freeze({
  getState: () => state,
  persist,
  render,
  showToast,
